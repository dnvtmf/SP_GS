...
def readNerfiesCameras(path):
    ...
    if name.startswith('vrig'):
        ...
        val_img = []
        ratio = 0.5
    print(f"[Nerfies] name={name}, ratio={ratio}, train={len(train_img)}, val={len(val_img)}")
...
def readNerfiesInfo(path, eval):
    ...
    if not os.path.exists(ply_path):
        ...
        storePly(ply_path, xyz, SH2RGB(shs) * 255)
    try:
        print(f'Load ply from {ply_path}')
        pcd = fetchPly(ply_path)
    except:
        pcd = None
...
def readHyperDataInfos(datadir, eval):
    print('begin load')
    from scene.hyper_loader import format_hyper_data, Load_hyper_data
    ratio = 0.5 if 'vrig' in datadir else 1.0
    train_cam_infos = Load_hyper_data(datadir, ratio, split="train")
    test_cam_infos = Load_hyper_data(datadir, ratio, split="test")
    print(f"load finished, ratio={ratio}, size={train_cam_infos.w} x {train_cam_infos.h}")
    ...
    scene_info = SceneInfo(point_cloud=pcd,
        train_cameras=[train_cam_infos[i] for i in range(len(train_cam_infos))],
        test_cameras=[test_cam_infos[i] for i in range(len(test_cam_infos))],
        nerf_normalization=nerf_normalization,
        ply_path=ply_path
    )
    return scene_info
