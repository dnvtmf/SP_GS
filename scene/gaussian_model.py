...
class GaussianModel:
    ...
    def split_points(self, selected_pts_mask, N=2):
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2, other_params=()):
        ...
        self.split_points(selected_pts_mask, N)
        for module in other_params:
            module.split_points(selected_pts_mask, N)
        prune_filter = torch.cat(
            (selected_pts_mask, torch.zeros(N * selected_pts_mask.sum(), device="cuda", dtype=torch.bool)))
        self.prune_points(prune_filter)
        for module in other_params:
            module.prune_points(prune_filter)

    def clone_points(self, selected_pts_mask):
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent, other_params):
        # Extract points that satisfy the gradient condition
        selected_pts_mask = torch.where(torch.norm(grads, dim=-1) >= grad_threshold, True, False)
        selected_pts_mask = torch.logical_and(selected_pts_mask,
            torch.max(self.get_scaling, dim=1).values <= self.percent_dense * scene_extent)
        self.clone_points(selected_pts_mask)
        for module in other_params:
            module.clone_points(selected_pts_mask)

    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size, other_params):
        grads = self.xyz_gradient_accum / self.denom
        grads[grads.isnan()] = 0.0

        self.densify_and_clone(grads, max_grad, extent, other_params)
        self.densify_and_split(grads, max_grad, extent, 2, other_params)
        ...
        for module in other_params:
            module.prune_points(prune_mask)
