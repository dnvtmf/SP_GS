...
class SuperpointModel(nn.Module):
    ...
    def loss(self, points_c: Tensor, points_t: Tensor, delta_r: Tensor, delta_t: Tensor, loss_aux, t=None):
        G, neighbor, sp_delta_t, sp_delta_r = loss_aux
        re_sp_delta_r = get_superpoint_features(delta_r, neighbor, G, self.num_superpoints)
        re_sp_delta_t = get_superpoint_features(delta_t, neighbor, G, self.num_superpoints)
        losses = {
            're_xyz': self.property_reconstruct_loss(G, neighbor, points_t),
            're_rot': F.mse_loss(re_sp_delta_r, sp_delta_r),
            're_off': F.mse_loss(re_sp_delta_t, sp_delta_t),
        }
        if t is not None:
            with torch.no_grad():
...
class DeformModel:
    ...
    def step(self, xyz, time_emb, use_mlp=True):
        if use_mlp:
            ...
            G, neighbor = self.sp_model.calc_association_matrix(xyz)
            sp_delta_xyz, sp_delta_r = self.sp_deform(self.sp_model.sp_xyz, time_emb)
            delta_xyz = torch.sum(G[:, :, None] * sp_delta_xyz[neighbor], dim=1)
            delta_r = torch.sum(G[:, :, None] * sp_delta_r[neighbor], dim=1)
            # delta_xyz = 0.5 * (sp_delta_xyz[self.sp_model.p2sp] + delta_xyz)
            # delta_r = 0.5 * (sp_delta_r[self.sp_model.p2sp] + delta_r)
        else:
            sp_delta_xyz, sp_delta_r = self.sp_model.interp_deform(time_emb)
            delta_xyz = self.sp_model.sp_delta_t[self.sp_model.p2sp]
            delta_r = self.sp_model.sp_delta_r[self.sp_model.p2sp]
            G, neighbor = None, None
        return (delta_xyz, delta_r, 0), (G, neighbor, sp_delta_xyz, sp_delta_r)
