...
def loadCam(args, id, cam_info, resolution_scale):
    ...
    if getattr(args, 'camera_noise_r', 0) > 0:
        from lietorch import SO3
        nR = SO3.exp(torch.randn(3) * args.camera_noise_r).matrix().numpy()[:3, :3]
        R = nR @ cam_info.R
    else:
        R = cam_info.R
    if getattr(args, 'camera_noise_t', 0) > 0:
        T = cam_info.T + np.random.randn(3) * args.camera_noise_t
    else:
        T = cam_info.T
